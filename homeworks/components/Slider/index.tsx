...
const SliderLeftButton = ({ className }: { className?: string }) => {
  return (
    <button className={"w-[80px] h-[80px] rounded-full border-[1px] border-borderPrimary flex items-center justify-center bg-white/10 " + className}>
      <ArrowLeftIcon />
    </button>
  );
};
    ...
    <div className={"flex gap-4 items-center " + className}>
      {footerLength.map((_, index) => {
        return active === index ? (
          ...
          <div key={index} className={"w-[15px] h-[15px] rounded-full" + ` ${color === "white" ? "bg-white" : "bg-primary"}`}></div>
        );
      })}
    </div>
  );
